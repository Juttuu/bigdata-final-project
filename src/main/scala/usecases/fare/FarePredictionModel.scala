...
object FarePredictionModel {
  val LabelColumn = "fare_amount"
  val RawFeaturesColumn = "raw_features"
  val FeaturesColumn = "features"
  val PredictionColumn = "predicted_fare_amount"
  ...
  def prepareDataset(featuredTrips: DataFrame): DataFrame = {
    ...
    featuredTrips
      ...
      .filter(col(LabelColumn) >= 2.5 && col(LabelColumn) <= 200.0)
  ...
  def samplePredictions(predictions: DataFrame, limit: Int = 10): DataFrame = {
    predictions.select(
      col(LabelColumn).as("actual_fare_amount"),
  ...
  private def validateColumns(dataFrame: DataFrame, requiredColumns: Seq[String]): Unit = {
    ...
    if (missingColumns.nonEmpty) {
      throw new IllegalArgumentException(
        s"Missing columns for fare prediction: ${missingColumns.mkString(", ")}"
      )
    }
  }
}
